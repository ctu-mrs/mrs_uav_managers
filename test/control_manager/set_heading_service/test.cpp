...
bool Tester::test(void) {
  ...
  {
    auto [success, message] = uh_->setHeading(0.5);

    if (!success) {
      RCLCPP_ERROR(node_->get_logger(), "set heading service failed with message: '%s'", message.c_str());
      return false;
    }
  }
  ...
  {
    auto [success, message] = uh_->setHeading(-3.14);
  ...
  {
    auto [success, message] = uh_->setHeading(0.0);
