...
bool Tester::test(void) {
  ...
  {
    auto [success, message] = uh_->setHeadingRelative(0.5);

    if (!success) {
      RCLCPP_ERROR(node_->get_logger(), "set heading relative failed with message: '%s'", message.c_str());
      return false;
    }
  }

  {
    auto [success, message] = uh_->setHeadingRelative(3.14);
  ...
  {
    auto [success, message] = uh_->setHeadingRelative(0);
