...
bool Tester::test(void) {
  ...
  const std::string old_gains = uh_->sh_gain_manager_diag_.getMsg()->current_name;
...

  {
    ...
    auto [success, message] = uh_->setGains(desired_gains);
  ...
  if (uh_->sh_gain_manager_diag_.getMsg()->current_name != old_gains) {
