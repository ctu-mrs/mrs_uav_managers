...
def generate_test_description():
    ...
    # the RTK path requires a static tf from the FCU to the RTK antenna, which a real platform
    # provides in its description but the simulator does not
    ld.add_action(
            launch_ros.actions.Node(
                package='tf2_ros',
                executable='static_transform_publisher',
                name='rtk_antenna_static_tf',
                output="screen",
                arguments=[
                    '--x', '0', '--y', '0', '--z', '0.1',
