...
namespace mrs_uav_managers
{
  ...
  namespace uav_manager
  {
    ...
    void UavManager::timerLanding(const ros::TimerEvent& event)
    {
      ...
      } else if (current_state_landing_ == LANDING_STATE)
      {
        ...
        if (_landing_tracker_name_ == sh_control_manager_diag_.getMsg()->active_tracker)
        {
          ...
          thrust_mass_estimate_ = mrs_lib::quadratic_thrust_model::thrustToForce(_motor_params_, desired_throttle) / _g_;
          ...
          if (((thrust_mass_estimate_ < _landing_cutoff_mass_factor_ * landing_uav_mass_) || desired_throttle < 0.01))
          {
    ...
    void UavManager::timerMaxThrust(const ros::TimerEvent& event)
    {
      ...
      if (desired_throttle >= _maxthrust_max_thrust_)
      {
        ...
        if (!maxthrust_above_threshold_)
        {
          ...
          ROS_WARN_THROTTLE(1.0, "[UavManager]: max thrust exceeded threshold (%.2f/%.2f)", desired_throttle, _maxthrust_max_thrust_);

        } else
        {

          ROS_WARN_THROTTLE(0.1, "[UavManager]: thrust over threshold (%.2f/%.2f) for %.2f s", desired_throttle, _maxthrust_max_thrust_,
      ...
      if (maxthrust_above_threshold_ && (ros::Time::now() - maxthrust_first_time_).toSec() > _maxthrust_ungrip_timeout_)
      {
        ...
        ROS_WARN_THROTTLE(1.0, "[UavManager]: thrust over threshold (%.2f/%.2f) for more than %.2f s, ungripping payload", desired_throttle,
      ...
      if (maxthrust_above_threshold_ && (ros::Time::now() - maxthrust_first_time_).toSec() > _maxthrust_eland_timeout_)
      {
        ...
        ROS_ERROR_THROTTLE(1.0, "[UavManager]: thrust over threshold (%.2f/%.2f) for more than %.2f s, calling for emergency landing", desired_throttle,
    ...
    void UavManager::timerDiagnostics(const ros::TimerEvent& event)
    {
      ...
      if (sh_odometry_diagnostics_.hasMsg())
    ...
    std::tuple<bool, std::string> UavManager::midairActivationImpl(void)
    {
      ...
      // the controller will output hover thrust and "leveled" desired orientation with the currend heading
      ...
      // this will cause the Control Manager to output something else than min-thrust
